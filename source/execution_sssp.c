...
*****************************************************************************
* execution_sssp.c
*      Implementation of the algorithm control flow for Single-Source
*      Shortest Path.
*****************************************************************************/
...
vertexprop_t execution_scalar_reduce_op_sssp(vertexprop_t a, vertexprop_t b)
{
    return (a.u < b.u ? a : b);
}
...
void execution_initialize_sssp(void)
{
    ...
    if (0 == spindleGetGlobalThreadID())
        graph_vertex_props[graph_param_root_vertex].u = 0ull;
    
    // activate the search root
...
void execution_impl_sssp(void)
{
    ...
    while(1)
    {
        ...
        if (use_nowedge_pull)
        {
            perform_edge_pull_phase_sssp(graph_edges_gather_list_bufs_numa[spindleGetTaskID()], graph_edges_gather_list_counts_numa[spindleGetTaskID()]);
        }
        else
        {
            ctr_wedge += 1ull;
            perform_edge_pull_phase_sssp_with_wedge_frontier(graph_edges_gather_list_bufs_numa[spindleGetTaskID()], graph_edges_gather_list_counts_numa[spindleGetTaskID()], graph_wedge_frontier_numa[spindleGetTaskID()], graph_wedge_frontier_count_numa[spindleGetTaskID()]);
        }
        ...
        if (0 == spindleGetGlobalThreadID())
        {
            edge_pull_op_idempotent_merge_with_merge_buffer(graph_vertex_merge_buffer, (uint64_t)spindleGetGlobalThreadCount() * 32ull, graph_vertex_props, execution_scalar_reduce_op_sssp);
        }
